...
import React, { useEffect, useState } from 'react';
import {
  Calendar, Clock, Users, Star, CheckCircle, User, Phone, Mail,
  MapPin, Briefcase, Cake
} from 'lucide-react';
...
function App() {
  ...
  // ----- ENV (normalize & validate) -----
  // IMPORTANT: set these in Netlify/Vercel environment variables (VITE_ prefix is required for Vite builds)
  const RAW_API = import.meta.env.VITE_API_URL as string | undefined; // example: https://api.pulseandpause.in
  const RZP_KEY = (import.meta.env.VITE_RAZORPAY_KEY_ID as string) || "";

  // Normalize API_BASE: remove trailing slash and ensure it starts with http/https
  const API_BASE = RAW_API ? RAW_API.replace(/\/+$/, '') : "";
  const API_OK = API_BASE.startsWith('http'); // simple check
  ...
  useEffect(() => {
    ...
    s.async = true;
    document.body.appendChild(s);
    // don't remove on cleanup to keep cached script
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ----- safety checks -----
      if (!API_BASE) {
        throw new Error('VITE_API_URL is not set. Set VITE_API_URL="https://api.pulseandpause.in" in Netlify/Vercel env and rebuild.');
      }
      if (!API_OK) {
        throw new Error(`VITE_API_URL must start with https:// or http:// — current value: ${API_BASE}`);
      }
      if (!RZP_KEY) {
        throw new Error('VITE_RAZORPAY_KEY_ID is not set. Add it in your frontend env and rebuild.');
      }

      // Debug: show exact endpoints in console (useful for redirect bugs)
      console.log('[frontend] Using API_BASE =', API_BASE);

      // 1) Save registration
      const registerUrl = `${API_BASE}/api/register`;
      console.log('[fetch] POST', registerUrl);
      ...
      if (!regRes.ok) {
        const msg = regData?.message || `Registration failed (status ${regRes.status})`;
        throw new Error(msg);
      }
      if (!regData?.success) {
        throw new Error(regData?.message || 'Registration failed (no success flag)');
      }

      const registrationId = regData?.data?.id ?? regData?.data?._id ?? null;
      console.log('[frontend] registrationId =', registrationId);

      // 2) Create Razorpay order
      // Send amount in paise. If price changes in UI, compute accordingly.
      const amountRupees = 99; // keep as rupees here
      const amountPaise = Math.round(amountRupees * 100); // 9900
      const createOrderUrl = `${API_BASE}/api/create-order`;
      console.log('[fetch] POST', createOrderUrl, 'body:', { amount: amountPaise, registrationId });

      const orderRes = await fetch(createOrderUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount: amountPaise, registrationId }),
      });

      const order = await orderRes.json().catch(() => null);
      if (!orderRes.ok) {
        const msg = order?.message || `Order creation failed (status ${orderRes.status})`;
        throw new Error(msg);
      }
      if (!order || !(order.id || order.order_id || order.data?.id)) {
        throw new Error('Order creation response did not include an order id.');
      }

      const orderId = order.id || order.order_id || order.data?.id;
      const orderAmount = order.amount ?? amountPaise;
      const orderCurrency = order.currency ?? 'INR';
      console.log('[frontend] orderId =', orderId, 'amount=', orderAmount, 'currency=', orderCurrency);

      if (!window.Razorpay) {
        throw new Error('Razorpay SDK not loaded. Confirm the checkout.js script is reachable.');
      }

      // 3) Open Razorpay checkout
      const options: any = {
        key: RZP_KEY, // publishable key (set in Netlify)
        amount: orderAmount,
        ...
        handler: async function (response: any) {
          try {
            ...
            console.log('[fetch] POST', verifyUrl, 'body:', { ...response, registrationId });
  ...
  const missingEnv = !API_BASE || !RZP_KEY;

  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-800">
      {/* Top environment warning banner */}
      {missingEnv && (
        <div className="w-full bg-red-600 text-white p-3 text-center">
          <strong>Environment variables missing:</strong>
          {!API_BASE ? ' VITE_API_URL is not set.' : ''}
          {!RZP_KEY ? ' VITE_RAZORPAY_KEY_ID is not set.' : ''}
          {' '}Set them in Netlify/Vercel & rebuild the site. (VITE_ prefix required for Vite)
        </div>
      )}
      ...
      {/* rest of your UI unchanged (hero, speakers, forms etc.) */}
      {/* For brevity I reuse your same JSX; keep the existing large markup you provided */}
      {/* --- */}
      {/* Hero Section and content: copy the same JSX you already had here — unchanged */}
      {/* (Everything from your original file after the header should be placed here verbatim) */}
      {/* ... */}
      <main>
        {/* Keep all the sections exactly as in your original file (hero, speakers, features, contact, modals) */}
        {/* To keep this example concise, I left the rest of the JSX unchanged, but in your code paste the full JSX from your original file after the header above */}
      </main>
